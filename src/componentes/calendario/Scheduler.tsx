...
import { DiaScheduler } from "./DiaScheduler";
    ...
    >
      ...
      >
        ...
        <Typography color="primary" variant="h6">
          {fechaActual.format("D MMMM, YYYY")}
        </Typography>
        <Button
          variant="text"
...
const Calendario = ({ fechaActual, toggleDialog }) => {
  ...
  const obtenerDiasAMostrar = useCallback(() => {
    ...
    for (
      let day = moment(primerdiaDelMes);
      day.isSameOrBefore(ultimoDiaMes);
      day.add(1, "day")
    ) {
      diasIteracion.push(moment(day));
    }

    const primerdiaDelSiguienteMes = moment(fechaActual)
      .add(1, "month")
      .startOf("month");
    ...
    if ((longitudDeDias / 7) % 1 !== 0) {
      for (
        let day = moment(primerdiaDelSiguienteMes);
        day.day() <= 35 - longitudDeDias;
        day.add(1, "day")
      ) {
        diasIteracion.push(moment(day));
      }
      ...
      <Stack display="grid" gridTemplateColumns="repeat(7, 1fr)" py={1}>
        {diasDeLaSemana.map((dia, index) => (
          ...
          >
            <Typography
              variant="caption"
              color={SincoTheme.palette.text.secondary}
            >
              {dia}
            </Typography>
          </Stack>
...
export const Scheduler: React.FC = () => {
  ...
  const handleMostrarDia = useCallback(() => {
    setMostrarDia(true);
  }, []);
    ...
    >
      ...
      >
        <Box display="flex" alignItems="center" justifyContent="center" gap={1}>
          ...
          <Typography variant="h6">Calendario de eventos</Typography>
        </Box>
      ...
      <CambioDeFecha
        ...
        cambiarFechaActual={cambiarFechaActual}
      />
      {!mostrarDia && (
        <Calendario fechaActual={fechaActual} toggleDialog={toggleDialog} />
      )}
      {mostrarDia && <DiaScheduler open={open} toggleDialog={toggleDialog} />}
